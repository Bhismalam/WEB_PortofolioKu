...
const renderSkillIcon = (iconName: string) => {
  switch (iconName) {
    ...
    case 'Atom': return <Atom className="w-6 h-6 text-blue-400" />;
    ...
    default: return <Terminal className="w-6 h-6 text-blue-400" />;
  }
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <ScrollReveal>
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h2 className="text-3xl sm:text-5xl font-extrabold text-white tracking-tight">
              {t.title} <span className="gradient-text-blue">{t.titleHighlight}</span>
            </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredSkills.map((skill, index) => (
            <ScrollReveal key={skill.id} delay={(index % 4) * 100}>
              <TiltCard className="glass-card rounded-2xl p-6 relative overflow-hidden group hover:border-blue-400/40 h-full flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[11px] font-mono-tech px-2.5 py-1 rounded-md bg-white/5 text-gray-300 border border-white/10">
                      {language === 'en' ? (skill.experience_years_en || skill.experience_years.replace('Thn', 'Yrs')) : skill.experience_years}
                    </span>
                  </div>
                  ...
                  <p className="text-xs text-gray-400 capitalize mb-4 font-mono-tech">
                    {language === 'en' ? 'Category' : 'Kategori'}: {skill.category}
                  </p>
                </div>
                ...
                <div className="w-full">
                  <div className="flex justify-between items-center text-xs mb-1.5 font-mono-tech">
                    ...
                    <span className="text-blue-400 font-bold">{skill.proficiency_level}%</span>
                  </div>
                  <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden p-0.5">
                    <div 
                      className="h-full rounded-full bg-gradient-to-r from-blue-500 via-blue-500 to-blue-400 transition-all duration-1000 ease-out"
                ...
                <div className="absolute -bottom-10 -right-10 w-24 h-24 bg-blue-400/10 rounded-full blur-xl group-hover:bg-blue-500/20 transition-all pointer-events-none"></div>
              </TiltCard>
            </ScrollReveal>
          ))}
        </div>
